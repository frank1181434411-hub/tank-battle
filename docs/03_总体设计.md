...
| EasyAI / NormalAI / HardAI | 敌方 AI 策略类 | EasyAI 和 NormalAI 已有初步实现，HardAI 仅有声明 |
| MapEditor | 地图编辑 | 计划新增 |
...
- AI 层：已建立 `AIController`、`AIContext`、`TankCommand` 和三类敌方 AI 类型；`EasyAI` 和 `NormalAI` 已有初步 `.cpp` 实现，`NormalAI` 已接入当前游戏测试流程，`HardAI` 待实现。
...
计划结构：后续可增加 `World`、`MapEditor` 和 `UI`，并继续完善已经建立的 `AIController` 与 `NetworkSession`。其中 `UI` 计划先实现 HUD，用于显示玩家血量、剩余敌人数和弹药强化状态。
...
当前数据流为输入更新玩家，`Game` 构造 `AIContext` 后调用敌方 AI 生成 `TankCommand`，再由敌人执行移动、转向和射击；`Game` 在更新前组装坦克与基地阻挡矩形列表，供玩家和敌人移动时判断；炮弹集合统一在 `Game` 中碰撞判定，命中后修改实体生命、破坏砖墙或删除炮弹。网络模式下预留客户端发送输入、主机更新世界并发送快照、客户端应用快照的流程，但尚未完成双端实机验证。
