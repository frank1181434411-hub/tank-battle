...
- 地图层：`Tile` 和 `Map` 已有基础实现，支持文本地图加载、瓦片查询、出生点记录和绘制接口；尚未接入 `Game` 主流程和碰撞系统。
...
计划结构：后续可增加 `World`、`AIController`、`MapEditor`、`NetworkSession` 和 `UI`，并把当前 `Map` 接入 `World` 或 `Game`。
