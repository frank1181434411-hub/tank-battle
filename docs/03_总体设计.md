...
| EasyAI / NormalAI / HardAI | 敌方 AI 策略类声明 | 已存在，未实现 `.cpp` |
| MapEditor | 地图编辑 | 计划新增 |
...
- AI 层：已建立 `AIController`、`AIContext`、`TankCommand` 和三类敌方 AI 头文件，具体决策实现待完成。
...
当前数据流为输入更新玩家，敌人按固定方向和自动射击更新；`Game` 在更新前组装坦克与基地阻挡矩形列表，供玩家和敌人移动时判断；炮弹集合统一在 `Game` 中碰撞判定，命中后修改实体生命、破坏砖墙或删除炮弹。网络模式下预留客户端发送输入、主机更新世界并发送快照、客户端应用快照的流程，但尚未完成双端实机验证。
